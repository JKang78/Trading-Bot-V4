"""
Research only: re-test regime switching with a STRICTER BTC bear definition.

Bear = BTC below 200-day EMA AND down at least 10% over the last 8 weeks.

Policies compared on the same V2 history:
  1. long_only        - current live style (no bear switch)
  2. cash_strict      - skip new entries during strict bear
  3. short_strict     - long outside bear; short only during strict bear

Does NOT change the live bot.
"""
...
from ml_strategy import V2_PROFILE, KrakenCostModel, compute_strict_btc_bear_frame
from tune_long_history import CACHE_DIR, SYMBOLS, fetch_cc_hourly

OUT_DIR = Path(".")
LOG = Path("research_regime_switch_strict.log")
...
def describe_strict_bear(btc_data: pd.DataFrame) -> None:
    """Show how often the strict bear rule is on, by year."""
    frame = compute_strict_btc_bear_frame(btc_data)
    daily = frame.resample("1D").last().dropna(subset=["is_bear"])
    bear_days = int(daily["is_bear"].sum())
    total_days = len(daily)
    log(
        f"\nStrict bear coverage: {bear_days}/{total_days} days "
        f"({bear_days / total_days * 100:.1f}%)"
    )
    log(f"  {'year':6s} {'bear_days':>9s} {'pct':>6s}")
    daily = daily.copy()
    daily["year"] = daily.index.year
    for year, group in daily.groupby("year"):
        n_bear = int(group["is_bear"].sum())
        log(f"  {year:<6d} {n_bear:9d} {n_bear / len(group) * 100:5.1f}%")
...
def run_policy(
    ...
    *,
    ...
    log(f"\n══ Policy: {name} (regime_mode={regime_mode}) ══")
...
def main() -> int:
    ...
    describe_strict_bear(btc_data)

    policies = [
        # Baseline uses no regime switch (same as previous long_only).
        ("long_only", 0.0, None, "default"),
        ("cash_strict", 0.0, "cash", "strict"),
        ("short_strict", SELL_THR_SHORT, "short", "strict"),
    ]

    results: dict[str, pd.DataFrame] = {}
    for name, sell_thr, bear_policy, regime_mode in policies:
        results[name] = run_policy(
            name, data, btc_data, profile, cost_model,
            sell_thr=sell_thr, bear_policy=bear_policy, regime_mode=regime_mode,
        )

    log("\n════════════════ COMPARISON (strict bear) ════════════════")
    log(
        f"{'policy':12s} {'trades':>6s} {'exp':>8s} {'win%':>6s} "
        f"{'$end':>8s} {'profit':>9s} {'CAGR':>7s} {'worst':>8s}"
    )
    rows = []
    for name, _sell, _pol, _mode in policies:
        trades = results[name]
    ...
    cash_end = portfolio_sim(results["cash_strict"])["end"]
    short_end = portfolio_sim(results["short_strict"])["end"]
    log("\n── Verdict vs long_only ──")
    log(f"  cash_strict:  {cash_end - base_end:+,.0f} dollars on $1000 compound path")
    log(f"  short_strict: {short_end - base_end:+,.0f} dollars on $1000 compound path")
